...
Calculator.prototype.createDOM=function(){
	...
	var SQRT=String.fromCharCode(8730);// корінь квадратний
	...
	var lineButton6=[0,".","+"];
	var lineButton=null;
	for (var i = 1; i <=6; i++) {   //створюємо div для 6 блоків-ліній
		var divLine=document.createElement('div');
		divLine.classList.add('line');
		calc.appendChild(divLine);
	
	    if(i==1){
			for (var j= 1; j <=2; j++) { //створюємо input 2 для 1 блоку-лінії
				...
				if(j==1){
					input.classList.add('top'); //для першого задаємо клас top

				}
				else{
					input.classList.add('bottom'); //для другого(іншого) задаємо клас bottom
					input.setAttribute('value',0);  //і початкове значення 0
				}
	    }

	}
	else{
		switch(i){    //далі для інших блоків задаємо відповідні кнопки
			case 2:
...
Calculator.prototype.createEvent=function(){
	...
	calc.onclick=function(event){
		...
		if(!isNaN(btnValue)){ //перевірка чи кнопка число
				//alert('Цифра');
...
Calculator.prototype.initProp=function(){
	this.replace=false;  //  перезапис(чотири наступні властивості)
	this.memory=0;
	this.repeatOperation=false;
	this.operation='';
	this.SQRT=false;
...
Calculator.prototype.initClick=function(top,bottom){
	top.value='';
	bottom.value=0;
	this.initProp();
}
